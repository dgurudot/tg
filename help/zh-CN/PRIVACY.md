...
**修订日期: 2026年9月19日.** 本更新适用于其后的所有内容。如果本更新与下方隐私政策的先前文本存在差异，以本更新为准。如果本译文与 https://tradegrub.com/privacy-policy 上的英文版本之间存在任何差异，以英文版本为准。

**我们收集的其他信息（补充第1条）。** 除第1条所列信息外，我们还会收集您提供的以下信息：

- 您授权的券商连接信息，例如账户标识符、访问令牌或凭据，以及我们从您的券商检索的持仓、订单、余额和交易
- 您创建的图表、布局、脚本、策略、提醒、自选列表和设置
- 您为获得支持而发送给我们的消息，以及（如果您使用我们的 API）您的 API 密钥和请求活动

**信息出售（补充第2条）。** 我们不会出售您的个人信息。

**Cookie 和分析（取代第3条的最后一句）。** 我们使用分析工具来了解我们的服务是如何被使用的。您可以通过浏览器设置拒绝或删除 Cookie，第11条说明了具体方法，不过拒绝 Cookie 可能会限制部分功能。先前提到的单独 Cookie 政策不再适用。

**删除后的保留（补充第7条）。** 当您删除账户或要求我们删除您的信息时，我们会将其删除，但在满足法律和监管要求、用于审计以及防止服务被滥用（例如已被移除的账户反复滥用）所需的情况下，我们此后会保留的有限数据除外。我们仅保留这些目的所需的内容，并且仅将其用于这些目的。

**16. 我们如何处理 STRATEGIST（我们的 AI 助手）中的信息？**

**_简而言之：_** _Strategist 通过第三方 AI 提供商运行。发送什么内容、发送给谁，取决于您如何使用它。_

**使用 TradeGrub 积分。** 您的消息以及您添加的任何附件会通过我们的服务器发送至我们将请求路由到的 AI 提供商，以便其作出回应。我们的服务器会记录用于计费的使用信息，例如令牌数量、模型、费用、状态和时间。这些计费记录不包含您的消息或回复的文本。

**使用您自己的密钥。** 您的密钥存储在您设备上的浏览器中，不会发送到我们的服务器，也不会在您的设备之间同步。请确保该设备和浏览器配置文件的安全。您的请求从您的浏览器直接发送至您选择的 AI 提供商，并适用该提供商的隐私政策。

**对话历史记录和附件。** 您的对话历史记录保存在您设备上的浏览器中，不会上传给我们。附件仅用于与之一同发送的消息，我们不会存储。

**语音听写。** 如果您使用听写功能，您的浏览器自带的语音服务会将您的语音转换为文本。我们不会收到您的音频，但您的浏览器可能会根据其提供商的条款将音频发送至该服务。

请不要在 Strategist 中包含您不希望 AI 提供商处理的敏感信息。

**17. 我们如何处理券商连接和市场数据？**

**_简而言之：_** _我们使用您授权的访问权限来显示您的账户并下达您的订单。_

当您连接券商时，我们会使用您授权的访问权限来检索您的账户信息、持仓、订单和交易，并在您指示我们时下单。我们使用这些信息来提供您所要求的功能。我们使用专为此目的设计的保护措施来保护券商凭据和令牌，并且您可以随时断开与券商的连接。您的券商自己的条款和隐私政策适用于其持有的数据。向我们的服务提供数据的市场数据提供商列于第19条。

**18. 我们通过 API 和小组件收集什么？**

**_简而言之：_** _提供服务、应用限制和保障安全所需的技术信息。_

**API。** 如果您使用我们的 API，我们会使用您的 API 密钥并记录请求活动，例如端点、时间和请求量，以便验证请求、应用速率限制并保护服务。

**小组件。** 当小组件在网站或应用程序上加载时，与任何网络请求一样，我们的服务器会收到技术信息，例如访问者的 IP 地址、浏览器和设备信息以及发出请求的页面。我们使用这些信息来提供小组件、保障其安全并应用使用限制。嵌入小组件的网站所有者对其向自己的访问者发出的通知负责。

**19. 我们的服务提供商有哪些？**

**_简而言之：_** _我们使用第三方来运行服务。每一方都有自己的隐私政策。_

- **支付：** Stripe。
- **市场和参考数据：** Polygon、Twelve Data、Alpha Vantage、SEC EDGAR 和公共政府数据来源。
- **您的券商，** 当您连接它们时。

**20. 我们使用您的信息的法律依据是什么？**

**_简而言之：_** _如果您位于欧洲经济区或英国，我们依赖以下法律依据。_

我们使用您的信息来履行与您的合同，例如提供您的账户和您所要求的服务。我们出于我们的合法利益使用您的信息，例如保障服务的安全、防止欺诈和滥用，以及了解并改进服务的使用方式。在法律要求的情况下，我们依赖您的同意，例如营销电子邮件和非必要的 Cookie，并且您可以随时撤回同意。在我们必须遵守法律义务时，我们也会使用您的信息。

**21. 如果发生安全事件会怎样？**

**_简而言之：_** _在法律要求时，我们会通知您和主管机关。_

我们采用技术和组织措施来保护您的信息，但没有任何系统是完全安全的。如果安全事件影响到您的个人信息，我们将按照适用法律的要求通知受影响的人员和相关主管机关。
